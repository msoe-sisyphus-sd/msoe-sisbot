...
var ble_obj = {
    ...
    update_ip_address: function(ip_address_str) {
        console.log('Updated IP ADDRESS', ip_address_str, ip_address_str.split('.').map(function(i) {
            return +i;
        }));
...
<<<<<<< HEAD
    ...
    init: function(config, session_manager) {
        ...
        this.current_state = this.collection.findWhere({
            type: "sisbot"
        });
        ...
        plotter.onFinishTrack(function() {
            ...
            if (playlist_id != "false") {
                ...
                self.current_state.set('active_track', playlist.get_next_track({
                    start_rho: self.current_state.get('_end_rho')
                }));
            } else if (self.current_state.get('is_loop') != "true") {
                self.current_state.set('active_track', {
                    id: 'false'
                });
            }
        });
                ...
                if (newState == 'home_rho_failed') {
                    ...
                    var track_obj = {
                        verts: [{
                            ...
                            th: self.config.failed_home_th,
                            r: self.config.failed_home_rho
                        }],
                        vel: 1,
                        accel: 0.5,
                        thvmax: 0.5
    ...
    _connect() {
        ...
        try {
            this.serial.open(function(error) {
                ...
                self.set_brightness({
                    value: self.current_state.get("brightness")
                }, null);
                self.set_speed({
                    value: self.current_state.get("speed")
                }, null);
                            ...
                            _.each(playlist.get("sorted_tracks"), function(track_index, index) {
                                if (searching) {
                                    ...
                                    if (new_obj.firstR == 0 || new_obj.lastR == 0) {
                                        playlist.set({
                                            active_track_index: -1
                                        }); // so this track is allowed to reverse
    ...
    home: function(data, cb) {
        ...
        if (this._validateConnection()) {
            if (data) { // special instructions?
                ...
                if (data.clear_tracks) this.current_state.set({
                    active_playlist_id: "false",
                    active_track: {
                        id: "false"
                    }
                }); // we don't keep track of where we are at anymore
            }
    ...
    add_playlist: function(data, cb) {
        ...
        var playlist = this.collection.add(new_playlist, {
            merge: true
        });
    ...
    add_track: function(data, cb) {
        ...
        var track = this.collection.add(new_track, {
            merge: true
        });
...
>>>>>>> 9dccbd61a822796c521b37e57bca87a5e1d9487d
        ...
        if (this._validateConnection()) {
            if (this.current_state.get("is_homed") == "true") {
                ...
                if (track != undefined) {
                    if (this.current_state.get("is_homed") == "true") {
                        _.extend(data, {
                            start: self.current_state.get('_end_rho')
                        });
                        var track_obj = track.get_plotter_obj(data);
                        if (track_obj != "false") {
                            this._paused = false;
        ...
        if (playlist != undefined) {
            ...
            if (this.current_state.get("is_homed") == "true") {
                var track = playlist.get_next_track({
                    start_rho: this.current_state.get('_end_rho')
                });
                if (track != "false") {
                    this._play_track(track, cb);
                }
...
<<<<<<< HEAD
    ...
    factory_reset: function(data, cb) {
        ...
        this.current_state.set({
            is_available: "false",
            reason_unavailable: "resetting"
        });
        if (cb) cb(null, this.current_state.toJSON());
        var ls = spawn('./factory_reset.sh', [], {
            cwd: "/home/pi/sisbot-server/sisbot/",
            detached: true,
            stdio: 'ignore'
        });
        ls.on('error', (err) => {
    ...
    restart: function(data, cb) {
        ...
        this.current_state.set({
            is_available: "false",
            reason_unavailable: "restarting"
        });
        if (cb) cb(null, this.current_state.toJSON());
        var ls = spawn('./restart.sh', [], {
            cwd: "/home/pi/sisbot-server/sisbot/",
            detached: true,
            stdio: 'ignore'
        });
